...
const MacOSDock: React.FC<MacOSDockProps> = ({ 
  ...
  const padding = Math.max(8, baseIconSize * 0.12);
  const dockRadius = Math.max(16, baseIconSize * 0.4);

    return (
    <div 
      ref={dockRef}
      className={`relative overflow-visible transition-all duration-300 ${className}`}
      style={{
              ...
              {openApps.includes(app.id) && (
                <div 
                  ...
                  style={{
                    ...
                    borderRadius: '50%',
                    backgroundColor: isDark ? 'rgba(255, 255, 255, 0.8)' : 'rgba(0, 0, 0, 0.55)',
